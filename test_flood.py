...
############## Defined Globally as used in multiple tests ####################
...
def test_stations_highest_rel_level():
  """
  Tests the stations_highest_rel_level method of flood.py
  """
  # Defines number of stations to be retrieved
  N = 9

  # test stations_highest_rel_level on the stations list
  N_stations = stations_highest_rel_level(stations, N)

  # Check N_stations is of length N
  assert len(N_stations) == N

  # Initilises last relative level as a large value
  last_rel_level = 10000000000
  for station in N_stations:
    # Get relavtive water level for station
    rel_level = station.relative_water_level()
    # Checks relative level calculated is roughly the vaule expected
    assert abs(rel_level - station_data_by_name[station.name][2]) < 0.01
    # Checks N_stations is ordered descending by relative level
    assert rel_level <= last_rel_level
    last_rel_level = rel_level